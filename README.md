...
- 공개 다시보기와 개별 캐치 링크를 지원합니다. 캐치스토리 링크는 SOOP가 해당 주소에서 선택한 개별 캐치로 안전하게 이동한 뒤 기존 캐치 다운로드 경로를 사용하며, 캐치스토리 목록 전체를 임의로 이어 붙이지 않습니다. 로그인·별도 시청 권한 필요, 암호화, 라이브, 타임라인 불연속, 분리 오디오 rendition 등 현재 처리하지 못하는 형식은 오류로 안내합니다.
